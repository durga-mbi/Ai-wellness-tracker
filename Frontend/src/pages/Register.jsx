...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.success) {
      ...
      setTimeout(() => navigate("/survey"), 2000); // Navigate to survey ritual
    } else {
        ...
        <div className="bg-white border border-gray-100 p-8 rounded-[32px] shadow-xl relative">
          {(error || success) && (
            <motion.div 
              ...
              className={`${success ? 'bg-green-50 border-green-100 text-green-600' : 'bg-red-50 border-red-100 text-red-600'} border p-3 rounded-xl text-[9px] font-bold uppercase tracking-widest mb-6 flex items-center gap-3`}
            >
              <span className={`w-4 h-4 rounded-full flex items-center justify-center text-[9px] ${success ? 'bg-green-100' : 'bg-red-100'}`}>
                {success ? '✓' : '!'}
              </span>
              {error || success}
            </motion.div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="space-y-2">
              <div className="flex justify-between items-center ml-1">
                ...
                {fieldErrors.name && <span className="text-[8px] font-black italic uppercase text-red-500 tracking-tighter">{fieldErrors.name}</span>}
              </div>
              <div className={`relative group transition-all ${fieldErrors.name ? 'ring-1 ring-red-500 rounded-xl' : ''}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <div className="flex justify-between items-center ml-1">
                    ...
                    {fieldErrors.email && <span className="text-[7px] font-black italic uppercase text-red-500 tracking-tighter">{fieldErrors.email}</span>}
                </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center ml-1">
                    <label className="text-[9px] font-bold uppercase tracking-widest text-gray-500">Mobile</label>
                    {fieldErrors.mobile && <span className="text-[7px] font-black italic uppercase text-red-500 tracking-tighter text-right">{fieldErrors.mobile}</span>}
                </div>
                <div className={`relative group transition-all ${fieldErrors.mobile ? 'ring-1 ring-red-500 rounded-xl' : ''}`}>
                  <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-gray-500 group-focus-within:text-black transition-colors">
            ...
            <button
              ...
              className="w-full py-3.5 bg-black text-white font-bold text-[10px] uppercase tracking-widest rounded-full hover:bg-gray-800 transition-all shadow-md mt-4 active:scale-95 duration-200"
            >
              {isSubmitting ? "Generating..." : "Join Hub"}
            </button>
          </form>

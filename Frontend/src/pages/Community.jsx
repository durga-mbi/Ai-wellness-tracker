      ...
      <section className="shrink-0 flex items-center justify-between">
        ...
        <div className="flex items-center gap-6 hidden md:flex">
           <div className="text-right">
             <p className="text-[8px] font-bold text-gray-500 uppercase tracking-widest italic leading-none">Total Posts</p>
      ...
      <div className="flex-1 overflow-y-auto pr-1 space-y-6 custom-scrollbar min-h-0 pb-10">
          ...
          {posts.map((post, idx) => (
            ...
            >
              ...
              <div className="flex items-center gap-4 pt-2">
                ...
                >
                   <div className={`p-2 rounded-lg transition-all ${post.likes > 0 ? 'bg-red-50 text-red-500' : 'bg-gray-50 text-gray-500 group-hover/like:text-black group-hover/like:bg-gray-100'}`}>
                      <HiHeart className={`w-4 h-4 transition-transform group-hover/like:scale-110 ${post.likes > 0 ? 'fill-current' : ''}`} />
                   </div>
                   <span className={`text-[10px] font-bold uppercase tracking-widest ${post.likes > 0 ? 'text-red-500' : 'text-gray-500 group-hover/like:text-black'}`}>{post.likes || 0}</span>
                </button>
              </div>
            </motion.div>
          ))}
        ...
        {isLoading && (
          <div className="py-20 flex flex-col items-center gap-4">
             ...
             <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest italic">Loading feed...</p>
          </div>
        )}

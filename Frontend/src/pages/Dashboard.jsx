...
const Dashboard = () => {
  ...
  return (
    <div className="flex-1 flex flex-col gap-8 h-full overflow-y-auto custom-scrollbar pr-2 pb-10">
      {/* Wellness Overview Hero */}
      <section className="relative overflow-hidden rounded-[48px] p-10 lg:p-14 flex flex-col lg:flex-row items-center justify-between min-h-[350px] shadow-sm border transition-all"
        style={{ background: C.surface, borderColor: `${C.outline}20` }}>

        {/* Soft decorative blobs */}
        <div className="absolute top-0 right-0 w-[40%] h-full bg-white opacity-40 blur-[100px] pointer-events-none rounded-full"></div>
        <div className="absolute bottom-0 left-0 w-[30%] h-[30%] bg-white opacity-30 blur-[80px] pointer-events-none rounded-full"></div>

        <div className="relative z-10 max-w-xl space-y-8 text-center lg:text-left">
          <motion.div
            initial={{ opacity: 0, scale: 0.95 }}
            animate={{ opacity: 1, scale: 1 }}
            className="inline-flex items-center gap-2 px-4 py-1.5 bg-white rounded-full shadow-sm border text-[10px] font-bold uppercase tracking-widest"
            style={{ borderColor: `${C.outline}40`, color: C.primary }}
          >
            <HiOutlineSparkles className="animate-pulse" /> Wellness Journey Overview
          </motion.div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-3 justify-center lg:justify-start">
              <p className="text-xs font-semibold px-3 py-1 bg-white/50 rounded-lg" style={{ color: C.textMut }}>
                Intensity: <span style={{ color: C.primary }} className="font-bold">{normalizedScore.toFixed(1)} / 10</span>
              </p>
            </div>
          </div>

          <p className="text-sm leading-relaxed max-w-md italic opacity-80" style={{ color: C.text }}>
            "{data?.weeklyAnalysis?.result ? `You've shown a ${data.weeklyAnalysis.result.toLowerCase()} pattern this week.` : "Continuing to observe your patterns."} Remember to take small breaks for your mind today."
          </p>
          ...
          <AnimatePresence>
            {!insightLoading && correlationData?.insight && (
              <motion.div
                ...
                className="relative mt-8 p-6 rounded-3xl shadow-xl overflow-hidden group border"
                style={{ background: C.primary, borderColor: `${C.outline}20` }}
              >
                <HiFire className="absolute top-4 right-4 text-3xl opacity-20 text-white animate-pulse" />
                <div className="flex flex-col relative z-10 gap-1">
                  <span className="text-[10px] font-bold text-white/70 uppercase tracking-widest flex items-center gap-1">
                    AI Sanctuary Insight <HiOutlineBolt className="text-yellow-300" />
                  </span>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {stats.map((stat, i) => (
          <motion.div
            ...
            className="bg-white border p-8 rounded-[40px] flex flex-col gap-6 hover:shadow-lg transition-all group"
          ...
          >
            ...
            <div>
              <p className="text-[11px] font-bold uppercase tracking-widest mb-1 opacity-60" style={{ color: C.textMut }}>{stat.label}</p>
              <h4 className="text-2xl font-extrabold tracking-tight" style={{ color: C.text, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>{stat.value}</h4>
            </div>
          </motion.div>
        ))}
      </section>
